...
void hash_write_record (record *, int, const char *);

void init_bucket (bucket *b, int position, int bucket_size) {
	b->position = position;
...
void hash_write_record (record *r, int position, const char *filename) {
	ofstream file;
...
void hash_add_record (record *record, map<int, bucket *> hash_table, int bucket_size, const char *filename) {
	...
	while (b->count >= b->bucket_size)
	       b = b->overflow;	
	hash_write_record (record, b->position + b->count, filename); 		//complete
	b->count++;
	if (b->count >= b->bucket_size) {
